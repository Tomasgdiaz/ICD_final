...
premier = read.csv("/home/infolab/Descargas/archive (1)/EPL_20_21.csv")
...
defensores=filter(premier, Position== "DF") %>%
  filter(Mins>3000, Perc_Passes_Completed>78)

ggplot(defensores, aes(Yellow_Cards, Mins)) + geom_point() + geom_text(aes(label=ifelse(defensores$Yellow_Cards>0,as.character(Name),'')),hjust=0,vjust=-0.5)
